...
class Tello:
    ...
    def __init__(self,
        ...
        if not self.threads_initialized:
            ...
            self.client_socket.setsockopt(socket.SOL_SOCKET, 25, interface.encode()) 
            self.client_socket.bind(('', Tello.CONTROL_UDP_PORT))
            response_receiver_thread = Thread(target=Tello.udp_response_receiver, args=(self.client_socket,interface,self.drones,))
            response_receiver_thread.daemon = True
            response_receiver_thread.start()

            # Run state UDP receiver on background
            state_receiver_thread = Thread(target=Tello.udp_state_receiver, args=(interface,self.drones))
        ...
        self.LOGGER.info("Tello instance was initialized. Host: '{}'. Port: '{}' Interface: '{}'.".format(host, Tello.CONTROL_UDP_PORT, interface))
    ...
    @staticmethod
    def udp_response_receiver(client_socket, interface, drones):
        ...
        while True:
            try:
                ...
                address = address[0]
                Tello.LOGGER.debug('Data received from {} at client_socket({})'.format(address, interface))
    ...
    @staticmethod
    def udp_state_receiver(interface, drones):
        ...
        state_socket.setsockopt(socket.SOL_SOCKET, 25, interface.encode())
        ...
        while True:
            try:
                ...
                address = address[0]
                Tello.LOGGER.debug('Data received from {} at state_socket({})'.format(address, interface))
    ...
    def get_udp_video_address(self) -> str:
        ...
        """
        address_schema = 'udp://@{ip}:{port}?localaddr={localip}'  # + '?overrun_nonfatal=1&fifo_size=5000'
        address = address_schema.format(ip=self.VS_UDP_IP, port=self.VS_UDP_PORT, localip=self.getLocalIp())
        return address
    ...
    def send_command_with_return(self, command: str, timeout: int = RESPONSE_TIMEOUT) -> str:
        ...
        self.LOGGER.info("Send command to Interface: {}: '{}'".format(self.interface,command))
        timestamp = time.time()

        self.client_socket.sendto(command.encode('utf-8'),0, self.address)
        ...
        self.LOGGER.info("Response from Interface:{} - {}: '{}'".format(self.interface, command, response))
        return response
